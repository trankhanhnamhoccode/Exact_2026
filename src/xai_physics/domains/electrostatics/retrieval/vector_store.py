...
from xai_physics.domains.electrostatics.retrieval.types import SchemaExample
...
class VectorExampleIndex:
    def __init__(self, examples: list[SchemaExample], embedding_model: EmbeddingModel | None = None):
        self.examples = examples
        self.embedding_model = embedding_model or TokenEmbeddingModel()
        self.example_vectors = {
            ex.id: self.embedding_model.embed(ex.problem + " " + " ".join(ex.tags))
            for ex in examples
        }

    def search(self, query: str, top_k: int = 5) -> list[tuple[SchemaExample, float]]:
        qvec = self.embedding_model.embed(query)
        scored = [(ex, cosine_sparse(qvec, self.example_vectors[ex.id])) for ex in self.examples]
